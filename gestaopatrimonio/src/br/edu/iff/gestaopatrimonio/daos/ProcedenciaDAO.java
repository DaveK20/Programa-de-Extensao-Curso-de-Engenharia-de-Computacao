...
public class ProcedenciaDAO {
		...
		public void cadastrarProcedencia(  LocalDate data, int patrimonioId,  int paisId, int estadoId, int cidadeId ) {
			try {
				...
				PreparedStatement ps = connection.prepareStatement("INSERT INTO procedencia (data, patrimonio_id, pais_id, estado_id, cidade_id) VALUES (?,?,?,?,?)");
		...
		public void atualizarProcedencia(int procedenciaId, Date novaData, int novoPatrimonioId,  int novoPaisId, int novoEstadoId, int novaCidadeId ) {
			try {
				PreparedStatement ps = connection.prepareStatement("UPDATE procedencia Set data = ?, patrimonio_id = ?, pais_id = ?, estado_id = ?, cidade id = ? WHERE id = ?");
